...
from .test_schema_flights import flight_schema
import random
import string
...
def test_create_flight_success(base_url, auth_headers, session_with_retries):
    ...
    # O podemos usar una aerolínea existente, pero para ser independientes, creamos una.
...
def test_get_flights_with_filters(base_url, auth_headers, session_with_retries):
    ...
    # (Nota: Esto podría ser redundante si ya existen vuelos, pero para ser independientes)
    airline_id = random_id()
    airline_data = {
        "id": airline_id,
        "name": "Test Airline",
    ...
    assert response.status_code == 200, f"Expected 200, got {response.statusCode}"
    flights = response.json()
    assert isinstance(flights, list)
...
def test_get_flight_by_id(base_url, auth_headers, session_with_retries):
    ...
    # Ahora obtenemos el vuelo por ID
    response = session_with_retries.get(
